// Copyright 2020 QMK
// SPDX-License-Identifier: GPL-2.0-or-later
#include QMK_KEYBOARD_H
#include "progmem.h"
#include "rgb_matrix.h"


enum layers{
	WIN_BASE,
	WIN_T,
	WIN_ALT,
	LWIN_DIS,
	WIN_FN,
	WIN_T_FN,
	WIN_ALT_FN,
	MAC_BASE,
	MAC_FN,
	ARROW_FN,
};

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
        [WIN_BASE] = LAYOUT_61(
		KC_ESC,  KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  KC_BSPC, 
		KC_TAB,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_LBRC, KC_RBRC, KC_BSLS, 
		KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT, KC_ENT, 
		KC_LSFT, KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,           KC_SLSH, KC_RSFT, 
		KC_LCTL, KC_LWIN, KC_LALT,                   KC_SPC,                                      KC_RALT, KC_APP,  KC_RCTL, MO(WIN_FN)),
	[WIN_FN]   = LAYOUT_61(
		KC_GRV,  KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12, KC_DEL, 
		_______, _______, _______, _______, _______, _______, KC_PSCR, KC_SCRL, KC_PAUS, _______, _______, UG_VALD, UG_VALU, UG_NEXT, 
		_______, _______, DF(MAC_BASE), _______, _______, _______, KC_INS, KC_HOME, KC_PGUP, _______, UG_SPDD, UG_SPDU, TG(ARROW_FN),
		QK_BOOT, _______, _______, _______, _______, NK_TOGG, KC_DEL, KC_END,  KC_PGDN, UG_HUEU, _______, _______,
		DF(WIN_ALT), TG(LWIN_DIS), _______, EE_CLR, _______, _______, _______, _______),
	[WIN_T]   = LAYOUT_61(
		KC_GRV, KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12, KC_DEL, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, MO(WIN_T_FN)),
	[WIN_T_FN]   = LAYOUT_61(
		KC_ESC,	KC_MYCM, KC_WHOM, KC_MAIL, KC_CALC, KC_MSEL, KC_MSTP, KC_MPRV, KC_MPLY, KC_MNXT, KC_MUTE, KC_VOLD, KC_VOLU, KC_BSPC, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, DF(MAC_BASE), _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, TG(LWIN_DIS), _______, _______, _______, _______, _______, _______, _______, _______, _______, TG(ARROW_FN), 
		DF(WIN_BASE), _______, _______, _______, _______, _______, _______, _______),
	[WIN_ALT]   = LAYOUT_61(
		KC_GRV, KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12, KC_DEL, 
		_______, _______, _______, _______, _______, _______, KC_PSCR, KC_SCRL, KC_PAUS, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, KC_INS, KC_HOME, KC_PGUP, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, KC_DEL, KC_END,  KC_PGDN, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, MO(WIN_ALT_FN)),
	[WIN_ALT_FN]   = LAYOUT_61(
		KC_ESC,	KC_MYCM, KC_WHOM, KC_MAIL, KC_CALC, KC_MSEL, KC_MSTP, KC_MPRV, KC_MPLY, KC_MNXT, KC_MUTE, KC_VOLD, KC_VOLU, KC_BSPC, 
		_______, _______, _______, _______, _______, _______, KC_Y, KC_U, KC_I, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, KC_H, KC_J, KC_K, _______, _______, _______, TG(ARROW_FN), 
		_______, _______, _______, _______, _______, _______, KC_N, KC_M, KC_COMM, _______, _______, _______, 
		DF(WIN_T), TG(LWIN_DIS), _______, _______, _______, _______, _______, _______),
        [MAC_BASE] = LAYOUT_61(
		KC_ESC,  KC_1, KC_2, KC_3, KC_4, KC_5, KC_6, KC_7, KC_8, KC_9, KC_0, KC_MINS, KC_EQL, KC_BSPC, 
		KC_TAB,  KC_Q, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_LBRC, KC_RBRC, KC_BSLS, 
		KC_CAPS, KC_A, KC_S, KC_D, KC_F, KC_G, KC_H, KC_J, KC_K, KC_L, KC_SCLN, KC_QUOT, KC_ENT, 
		KC_LSFT, KC_Z, KC_X, KC_C, KC_V, KC_B, KC_N, KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RSFT, 
		KC_LCTL, KC_LOPT, KC_LCMD, KC_SPC, KC_RCMD, KC_APP, KC_RCTL, MO(MAC_FN)),
        [MAC_FN]   = LAYOUT_61(
		KC_ESC, KC_1, KC_2, KC_3, KC_4, KC_5, KC_6, KC_7, KC_8, KC_9, KC_0, KC_MINS, KC_EQL, KC_BSPC, 
		KC_TAB, KC_Q, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_LBRC, KC_RBRC, KC_BSLS, 
		KC_CAPS, DF(WIN_BASE), KC_S, KC_D, KC_F, KC_G, KC_H, KC_J, KC_K, KC_L, KC_SCLN, KC_QUOT, TG(ARROW_FN), 
		KC_LSFT, KC_Z, KC_X, KC_C, KC_V, KC_B, KC_N, KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RSFT, 
		KC_LCTL, KC_LOPT, KC_LCMD, KC_SPC, KC_RCMD, KC_F1, KC_RCTL, KC_F2),
	[ARROW_FN]   = LAYOUT_61(
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,  
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, KC_UP, _______, 
		_______, _______, _______, _______, KC_LEFT, KC_DOWN, KC_RIGHT, _______),
	[LWIN_DIS]   = LAYOUT_61(
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,  
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, 
		_______, KC_NO, _______, _______, _______, _______, _______, _______),
};

extern uint8_t blink_cnt;
bool process_record_user(uint16_t keycode, keyrecord_t *record)
{
		switch (keycode) {
		case DF(WIN_BASE):
		case DF(MAC_BASE):
			if (record->event.pressed)
				{
					blink_cnt=1;
					set_single_persistent_default_layer(MAC_BASE);
				}
			return true;
		case UG_VALD:
			if (record->event.pressed)
				{
					dprintf("rgb_matrix_get_hue()=%d",rgb_matrix_get_hue());
					if(rgb_matrix_get_val()==0)
						blink_cnt=1;
				}
			return true;
		case UG_VALU:
			if (record->event.pressed)
				{
					dprintf("rgb_matrix_get_hue()=%d",rgb_matrix_get_hue());
					if(rgb_matrix_get_val()==RGB_MATRIX_MAXIMUM_BRIGHTNESS)
						blink_cnt=1;
				}
			return true;
		case UG_SPDU:
			if (record->event.pressed)
				{
					dprintf("rgb_matrix_get_speed()=%d",rgb_matrix_get_speed());
				}
			else
				{
					dprintf("rgb_matrix_get_speed()=%d",rgb_matrix_get_speed());
					if(rgb_matrix_get_speed()>=(RGB_MATRIX_DEFAULT_SPD+2*RGB_MATRIX_SPD_STEP))
						{
							blink_cnt=1;
							return false;
						}
				}
			return true;
		case UG_SPDD:
			if (record->event.pressed)
				{
					dprintf("rgb_matrix_get_speed()=%d",rgb_matrix_get_speed());
				}
			else
				{
					dprintf("rgb_matrix_get_speed()=%d",rgb_matrix_get_speed());
					if(rgb_matrix_get_speed()<=(RGB_MATRIX_DEFAULT_SPD-2*RGB_MATRIX_SPD_STEP))
						{
							blink_cnt=1;
							return false;
						}
				}
		}
			return true;
		

}

#ifdef CAPS_LOCK_LED_INDEX
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    if (host_keyboard_led_state().caps_lock) {
        RGB_MATRIX_INDICATOR_SET_COLOR(CAPS_LOCK_LED_INDEX, 255, 255, 255);
    } else {
        if (!rgb_matrix_get_flags()) {
           RGB_MATRIX_INDICATOR_SET_COLOR(CAPS_LOCK_LED_INDEX, 0, 0, 0);
        }
    }
	if (layer_state_is(LWIN_DIS)) {
		RGB_MATRIX_INDICATOR_SET_COLOR(59, 255, 255, 255);
	} else {
        if (!rgb_matrix_get_flags()) {
           RGB_MATRIX_INDICATOR_SET_COLOR(CAPS_LOCK_LED_INDEX, 0, 0, 0);
        }
    }

    if (layer_state_is(ARROW_FN)) {
    for (uint8_t row = 0; row < MATRIX_ROWS; ++row) {
        for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
            uint8_t index = g_led_config.matrix_co[row][col];

            if (index >= led_min && index < led_max && index != NO_LED) {
                keypos_t pos = (keypos_t){col, row};
                uint16_t keycode = keymap_key_to_keycode(ARROW_FN, pos);

                switch (keycode) {
                    case KC_LEFT:
                    case KC_DOWN:
                    case KC_RIGHT:
		    case KC_UP:
                        rgb_matrix_set_color(index, 255, 0, 0);
                        break;
                    default:
                        break;
                }
            }
        }
    }
}

    return true;
}
#endif
